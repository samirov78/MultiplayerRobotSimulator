	...
	bool estConcret, bool enLigne, char* id)
{
	...
	if (std::string(nom) != arbre_->NOM_SEGMENT)
	{
		...
		noeud->mettreAjourCentre();
		//3noeud->setPoints(point1, point2);
		chaineDeSauvegarde = noeud->toString();
					...
					elementXMLCourantLecture_ = elementXMLCourantLecture_->NextSiblingElement())
				{
					...
					if (typeNoeudString == arbre_->NOM_LIGNE) {
							...
							elementXMLSegment_ = elementXMLSegment_->NextSiblingElement())
						{
							...
							//3glm::dvec3 pointDepartLu;
							//3glm::dvec3 pointFinLu;

							/*3elementXMLSegment_->QueryDoubleAttribute("Depart_x", &pointDepartLu.x);
							...
							//3noeudSegment->setPoints(pointDepartLu, pointFinLu);
						...
						//3glm::dvec3 pointDepartLuMurPoteau;
						//glm::dvec3 pointFinLuMurPoteau;

						/*3elementXMLCourantLecture_->QueryDoubleAttribute("Depart_x", &pointDepartLuMurPoteau.x);
						...
						noeud->ID = id;
						//3noeud->setPoints(pointDepartLuMurPoteau, pointFinLuMurPoteau);
												...
												double angleRotation, glm::dvec3 pointDepart, glm::dvec3 pointFin, bool estFantome)
{
	...
	//3noeud->setPoints(pointDepart, pointFin);
	return noeud;
}
...
void FacadeModele::assignerPositionPourObjetSelOnline(double x, double y, double z)
{
	bool retour = false;
	int nombreNoeud = 0;
	NoeudAbstrait* noeud = GetNodeByID(mySelectedNodesIDs[0]);
	if (mySelectedNodesIDs.size() == 1)
	{
		initialiserEnTraitement();
		noeud->mettreAjourCentre();
		glm::dvec3 position = noeud->obtenirCentre();
...
void FacadeModele::PivoterObjetSelOnline(double deltaY)
{
	deltaY /= 2;
	int nombreNoeud = 0;
	NoeudAbstrait* noeud = GetNodeByID(mySelectedNodesIDs[0]);
	if ((mySelectedNodesIDs.size() == 1))
	{
		...
		if (noeud->obtenirType() == arbre_->NOM_LIGNE)
		{
			...
			for (unsigned int k = 0; k < noeud->obtenirNombreEnfants(); k++)
			{
...
{
	NoeudAbstrait * maFleche = GetNodeByID(Fleche_);
	...
	glm::dvec3 positionRobot = maFleche->obtenirPositionRelative();
	//positionRobot.z = 4.0;
	...
	double angleDepart = maFleche->obtenirRotationRelative()[0] - 90;
	...
	VisiteurVerifierCollision * visiteurVerColl = new VisiteurVerifierCollision();
	leRobot_->visiteurVerifierCollision_ = visiteurVerColl;
	visiteurVerColl->ptrLeRobot_ = leRobot_;
...
void FacadeModele::CreateRobot(std::string id)
{
	NoeudRobot* robot = (NoeudRobot*)arbre_->ajouterNouveauNoeud(arbre_->NOM_TABLE, arbre_->NOM_ROBOT);
	glm::dvec3 positionRobot = GetNodeByID(Fleche_)->obtenirPositionRelative();
	//positionRobot.z = 4.0;
	robot->assignerPositionRelative(positionRobot);

	robot->assignerRedimensionnementRelative(glm::dvec3(SCALE_ROBOT, SCALE_ROBOT, SCALE_ROBOT));

	double angleDepart = GetNodeByID(Fleche_)->obtenirRotationRelative()[0] - 90;
	robot->assignerRotationRelative(glm::dvec4(angleDepart, 0.0, 0.0, 1.0));

	//NoeudAbstrait* noeudRoueD1 = arbre_->ajouterNouveauNoeud(arbre_->NOM_ROBOT, arbre_->NOM_ROUE_DROITE);
	NoeudAbstrait* noeudRoueD1 = arbre_->creerNoeud(arbre_->NOM_ROUE_DROITE);
	robot->ajouter(noeudRoueD1);
	//NoeudAbstrait* noeudRoueG1 = arbre_->ajouterNouveauNoeud(arbre_->NOM_ROBOT, arbre_->NOM_ROUE_GAUCHE);
	NoeudAbstrait* noeudRoueG1 = arbre_->creerNoeud(arbre_->NOM_ROUE_GAUCHE);
	robot->ajouter(noeudRoueG1);
	noeudRoueD1->assignerRedimensionnementRelative(glm::dvec3(1.0, 1.0, 1.0));
	noeudRoueG1->assignerRedimensionnementRelative(glm::dvec3(1.0, 1.0, 1.0));

	noeudRoueD1->assignerPositionRelative(glm::dvec3(-3.8, 0.1, 3.0));
	noeudRoueG1->assignerPositionRelative(glm::dvec3(4.3, 0.1, 3.0));

	robot->setRoueDroite(noeudRoueD1);
	robot->setRoueGauche(noeudRoueG1);
	robot->bteEnglobante_ = new utilitaire::BteEnglobanteRobot;
	robot->mettreAjourBteEnglobante();
	//robot->visiteurVerifierCollision_ = new VisiteurVerifierCollision();

	VisiteurVerifierCollision * visiteurVerColl2 = new VisiteurVerifierCollision();
	robot->visiteurVerifierCollision_ = visiteurVerColl2;
	visiteurVerColl2->ptrLeRobot_ = robot;

	robot->typeCollision_ = AUCUNE;
	robot->objetCollision_ = AUCUN;
	visiteurDetectionNoeud = new VisiteurDetectionNoeud(robot);
	//millisecondes_ = 0;
	//compteurAffichageChrono_ = 0;
	//start = clock() ;
...
void FacadeModele::CreatePoteau(const char* data)
{
	...
	//3noeud->setPoints(glm::dvec3(std::atof(tokens[Depart_x].c_str()), std::atof(tokens[Depart_y].c_str()),
	//3	std::atof(tokens[Depart_z].c_str())), glm::dvec3(std::atof(tokens[Fin_x].c_str()),
	//3	std::atof(tokens[Fin_y].c_str()), std::atof(tokens[Fin_z].c_str())));
}
...
NoeudAbstrait* FacadeModele::CreateSegment(const char* data)
{
	std::string dataString(data);
	std::vector<std::string> tokens = utilitaire::split(data, '#');
	NoeudAbstrait* noeud = arbre_->creerNoeud(arbre_->NOM_SEGMENT);
	noeud->assignerAffiche(tokens[Affiche] == "1" ? true : false);
	noeud->assignerAffiche(tokens[Enregistrable] == "1" ? true : false);
	noeud->assignerAffiche(tokens[Selectionnable] == "1" ? true : false);
	...
	noeud->assignerRedimensionnementRelative(glm::dvec3(std::atof(tokens[Redim_x].c_str()),
		std::atof(tokens[Redim_y].c_str()), std::atof(tokens[Redim_z].c_str())));
	//3noeud->setPoints(glm::dvec3(std::atof(tokens[Depart_x].c_str()), std::atof(tokens[Depart_y].c_str()),
		//3std::atof(tokens[Depart_z].c_str())), glm::dvec3(std::atof(tokens[Fin_x].c_str()),
			//3std::atof(tokens[Fin_y].c_str()), std::atof(tokens[Fin_z].c_str())));
...
void FacadeModele::ModifyFleche(const char* data)
{
	std::string dataString(data);
	std::vector<std::string> tokens = utilitaire::split(data, '#');
	NoeudAbstrait* noeud = GetNodeByID(tokens[0]);
...
void FacadeModele::ModifyLine(const char* data)
{
	...
	for (int i = 0; i < min; i++)
	{
		...
		//3segmentNoeud->setPoints(glm::dvec3(std::atof(segmentData[Depart_x].c_str()), std::atof(segmentData[Depart_y].c_str()),
			//3std::atof(segmentData[Depart_z].c_str())), glm::dvec3(std::atof(segmentData[Fin_x].c_str()),
				//3std::atof(segmentData[Fin_y].c_str()), std::atof(segmentData[Fin_z].c_str())));
...
const char * FacadeModele::ChangerConfigurationObjetSelectionne(double x, double y, double angle, double scale)
{
	chaineConfig = "";
	int nombre = true;
	NoeudAbstrait* noeud = chercherNouedSelectionne(nombre);
	std::string lenoeud = noeud->toString();
	glm::dvec3 position = noeud->obtenirPositionRelative();
	...
	{
		if(noeud->obtenirType() == arbre_->NOM_MUR)
			ModifyMur(lenoeud.c_str());
		else if (noeud->obtenirType() == arbre_->NOM_POTEAU)
			ModifyPoteau(lenoeud.c_str());
		else if (noeud->obtenirType() == arbre_->NOM_LIGNE)
			ModifyLine(lenoeud.c_str());
		else if (noeud->obtenirType() == arbre_->NOM_FLECHE)
			ModifyFleche(lenoeud.c_str());
	...
	if (retour) 
	   { chaineConfig = "1@" + noeud->toString(); }
	else
	    { chaineConfig = "0@" + noeud->toString(); }

	return chaineConfig.c_str();
}
...
void FacadeModele::moveNodesOnline(char * IDsAndDebutFin)
{
	...
	for (int i = 0; i < nombreNoeuds; i++)
	{
		...
		if (nodesToMove[i]->obtenirType() == arbre_->NOM_LIGNE)
		{
			...
			for (unsigned int k = 0; k < nodesToMove[i]->obtenirNombreEnfants(); k++)
			{
...
bool FacadeModele::duplicateSelNodes(char * chaineDup)
{
	...
	for (int i = 0; i < IDsToDuplicate.size(); i++)
	{
		...
		if (noeudTemp->obtenirType() == arbre_->NOM_LIGNE)
		{
			...
			for (unsigned int k = 0; k < noeudTemp->obtenirNombreEnfants(); k++)
			{
				children->at(k)->accepterVisiteur(visiteurDeterminerCentreSelection);
			}
		}
...
int FacadeModele::getMySelectedNodesNumber()
{
	return mySelectedNodesIDs.size();
}
//SetRobotUsername
void FacadeModele::SetRobotUsername(char * username)
{
	std::string userString(username);
	username_ = userString;
	leRobot_->ID = userString;
}

const char * FacadeModele::getInfosRobot()
{
	chaineRobot_ = "";
	glm::dvec4 rotatioRelative = leRobot_->obtenirRotationRelative();
	std::string rotation = utilitaire::toString(rotatioRelative[0]) + "#" + utilitaire::toString(rotatioRelative[1])
		+ "#" + utilitaire::toString(rotatioRelative[2]) + "#" + utilitaire::toString(rotatioRelative[3]);

	glm::dvec3 positionRelative = leRobot_->obtenirPositionRelative();
	std::string position = utilitaire::toString(positionRelative[0]) + "#" + utilitaire::toString(positionRelative[1])
		+ "#" + utilitaire::toString(positionRelative[2]);
	chaineRobot_ = leRobot_->ID + "#" + position + "#" + rotation;
	return chaineRobot_.c_str();
}

//pour mettre a jour les infos du robot d un autre joueur
void FacadeModele::mettreAjourRobot(char * chaineRobot)
{
	std::string dataString(chaineRobot);
	std::vector<std::string> components = utilitaire::split(dataString, '#');
	std::string IDRobot = components[0];
	glm::dvec3 positionRelRobot;
	glm::dvec4 rotatioRelRobot;
	positionRelRobot[0] = std::atof(components[1].c_str()); 
	positionRelRobot[1] = std::atof(components[2].c_str()); 
	positionRelRobot[2] = std::atof(components[3].c_str());
	rotatioRelRobot[0] = std::atof(components[4].c_str());
	rotatioRelRobot[1] = std::atof(components[5].c_str());
	rotatioRelRobot[2] = std::atof(components[6].c_str());
	rotatioRelRobot[3] = std::atof(components[7].c_str());
	//localiser le robot sur la table
	NoeudAbstrait * autreRobot = GetNodeByID(IDRobot);

	//si le robot existe deja ON set simplement les attributs
	if (autreRobot != NULL)
	{
		mtx.lock();
		//autreRobot->ID = components[0];
		autreRobot->assignerPositionRelative(positionRelRobot);
		autreRobot->assignerRotationRelative(rotatioRelRobot);
		mtx.unlock();
	}
	else //Si le robot n existe pas deja
	{
		CreateRobot(components[0]);
		/*autreRobot = (NoeudRobot*)arbre_->ajouterNouveauNoeud(arbre_->NOM_TABLE, arbre_->NOM_ROBOT);
		autreRobot->assignerPositionRelative(positionRelRobot);
		autreRobot->assignerRotationRelative(rotatioRelRobot);*/
	}
}

const char * FacadeModele::getMySelectedNodesString()
{
	NoeudAbstrait * noeud = NULL;
	chaineDeSauvegarde = "";
	int size = mySelectedNodesIDs.size();
	if(size == 0)
		return chaineDeSauvegarde.c_str();
	for (int i = 0; i < size - 1; i++)
	{
		noeud = GetNodeByID(mySelectedNodesIDs[i]);
		if(noeud != NULL)
			chaineDeSauvegarde += noeud->toString() + "@";
	}
	if (size - 1 > 0)
	{
		noeud = GetNodeByID(mySelectedNodesIDs[mySelectedNodesIDs.size() - 1]);
		if (noeud != NULL)
			chaineDeSauvegarde += noeud->toString();
	}
	return chaineDeSauvegarde.c_str();
}
const char * FacadeModele::getDuplicatedNodesStr()
{
	NoeudAbstrait * noeud = NULL;
	chaineDeSauvegarde = "";
	std::cout << "nombre de dupliques C++: " << noeudsDuliques.size() << std::endl;
	if (noeudsDuliques.size() == 0)
	{
		chaineDeSauvegarde = "cancelled";
	}
	else
	{
		for (int i = 0; i < noeudsDuliques.size() - 1; i++)
		{
			chaineDeSauvegarde += noeudsDuliques[i]->toString() + "@";
		}
		chaineDeSauvegarde += noeudsDuliques[noeudsDuliques.size() - 1]->toString();
	}
	return chaineDeSauvegarde.c_str();
}

bool FacadeModele::ChangerConfigurationObjetSelOnline(double x, double y, double angle, double scale)
{
	NoeudAbstrait* noeud = GetNodeByID(mySelectedNodesIDs[0]);
	glm::dvec3 position = noeud->obtenirPositionRelative();
	glm::dvec4 rotation = noeud->obtenirRotationRelative();
	...
	assignerPositionPourObjetSelOnline(x, y, 0);
	noeud->changerScale(scale / echelle[0]);
	PivoterObjetSelOnline(2 * (angle - rotation[0]));
	bool retour = true;
	if (!noeud->estDansLaTable())
	{
		assignerPositionPourObjetSelOnline(position[0], position[1], position[2]);
		noeud->changerScale(echelle[0] / scale);
		PivoterObjetSelOnline(2 * (rotation[0] - angle));
		retour = false;
	}
	return retour;
...
void FacadeModele::initialiserFleches(char * fleche)
{
	std::cout << "initialiserFleches appele c++" << std::endl;
	std::string dataString(fleche);
	std::vector<std::string> components = utilitaire::split(dataString, '#');
	NoeudAbstrait * noeud = NULL;
	Fleche_ = components[0];
	for (int i = 1; i < components.size(); i++)
	{
			if (components[i] == "Fleche2")
			{
				fleche2 =(NoeudFleche *)arbre_->ajouterNouveauNoeud(arbre_->NOM_TABLE, arbre_->NOM_FLECHE, "Fleche2");
				fleche2->assignerPositionRelative(glm::dvec3(-42.0, 0.0, 1.0));
				fleche2->assignerRedimensionnementRelative(glm::dvec3(1.0, 1.0, 1.0));
				fleche2->tourner(180.0);
			}
			else if (components[i] == "Fleche3")
			{
				fleche3 = (NoeudFleche *)arbre_->ajouterNouveauNoeud(arbre_->NOM_TABLE, arbre_->NOM_FLECHE, "Fleche3");
				fleche3->assignerPositionRelative(glm::dvec3(-1.8, 15.0, 1.0));
				fleche3->assignerRedimensionnementRelative(glm::dvec3(1.0, 1.0, 1.0));
				fleche3->tourner(90.0);
			}
			else if (components[i] == "Fleche4")
			{
				fleche4 = (NoeudFleche *)arbre_->ajouterNouveauNoeud(arbre_->NOM_TABLE, arbre_->NOM_FLECHE, "Fleche4");
				fleche4->assignerPositionRelative(glm::dvec3(-1.8, -15.0, 1.0));
				fleche4->assignerRedimensionnementRelative(glm::dvec3(1.0, 1.0, 1.0));
				fleche4->tourner(270.0);
			}
			//lesFleches.push_back(noeud);
	}
}

void FacadeModele::EnleverFlechesOnline()
{
	std::cout << "EnleverFlechesOnline appelee" << std::endl;
	if (fleche2 != NULL)
		arbre_->chercher(arbre_->NOM_TABLE)->effacer(fleche2);
	else if (fleche3!=NULL)
		arbre_->chercher(arbre_->NOM_TABLE)->effacer(fleche3);
	else if (fleche4 != NULL)
		arbre_->chercher(arbre_->NOM_TABLE)->effacer(fleche4);

}

void FacadeModele::supprimerDernierNoeudOnline(char * id)
{
	std::string theID(id);
	NoeudAbstrait * noeud = GetNodeByID(theID);
	if (noeud != NULL)
	{
		arbre_->effacer(noeud);
		/*if (noeud->obtenirType() == arbre_->NOM_MUR)
		{
			arbre_->chercher(arbre_->NOM_TABLE)->effacer(noeud);
			NoeudAbstrait::compteurMur_--;
		}
		else if (noeud->obtenirType() == arbre_->NOM_LIGNE)
		{
