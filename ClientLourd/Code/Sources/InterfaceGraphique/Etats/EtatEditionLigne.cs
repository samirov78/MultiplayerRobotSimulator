                ...
                if ( !Edition.enTrace_ )
                {
                    ...
                    lock (Program.NoyauLock)
                    {
                        line = Marshal.PtrToStringAnsi(FonctionsNatives.ajouterNoeudSimple
                            (NOM_LIGNE, xDebut_, yDebut_, Program.OnLine, id));
                    }
                    if (Program.OnLine)
                ...
                else if ( k == Keys.Control )
                {
                    ...
                    lock (Program.NoyauLock)
                    {
                        line = Marshal.PtrToStringAnsi(FonctionsNatives.ajouterNoeudAvecScale
                            (NOM_SEGMENT, xDebut_, yDebut_, xFin_, yFin_, true, Program.OnLine));
                    }
                    if (Program.OnLine)
        ...
        public override void interpreterDeplacement(int x, int y)
        {
            ...
            if  ( curseurValide_ )
            {
                ...
                if ( Edition.enTrace_ )
                {
                    ...
                    {
                        line = Marshal.PtrToStringAnsi(FonctionsNatives.ajouterNoeudAvecScale
                            (NOM_SEGMENT, xDebut_, yDebut_, xFin_, yFin_, false, Program.OnLine, id));
                    }
