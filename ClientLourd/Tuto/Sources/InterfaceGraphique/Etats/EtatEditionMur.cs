        ...
        public override void interpreterClic(int x, int y, Keys k)
        {
            if ( curseurValide_ )
            {
                ...
                if ( !Edition.enTrace_ )
                {
                    ...
                    {
                        mur = Marshal.PtrToStringAnsi(FonctionsNatives.ajouterNoeudAvecScale(NOM_MUR, xDebut_, yDebut_, x, y, false, Program.edition.OnLine, id));
                        Button ggt = new Button();
                        ggt.Text = "hhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhh";
                ...
                else
                {
                    ...
                    lock (Program.NoyauLock)
                    {
                        ...
                        mur = Marshal.PtrToStringAnsi(FonctionsNatives.ajouterNoeudAvecScale(NOM_MUR, xDebut_, yDebut_, x, y, true,Program.edition.OnLine, id));
                    }
                    if (Program.edition.OnLine)
                        Program.SDispatcher.EditionConnectionM.SpreadCreateNode(mur, false);
                    else
            ...
            if ( curseurValide_  && Edition.enTrace_)
            {
                ...
                lock (Program.NoyauLock)
                {
                    ...
                    mur = Marshal.PtrToStringAnsi(FonctionsNatives.ajouterNoeudAvecScale(
                        NOM_MUR, xDebut_, yDebut_, x, y, false, Program.edition.OnLine, id));
                }
