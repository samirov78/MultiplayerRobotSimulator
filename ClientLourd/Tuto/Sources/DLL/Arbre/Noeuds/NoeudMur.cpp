...
void NoeudMur::changerScale(double scale)
{
	...
	//{
		...
		pointDepart_ = position - ((pointFin - pointDepart) / ancienneLongueur) * (nouvelleLongueur / 2);
		pointFin_ = position + ((pointFin - pointDepart) / ancienneLongueur) * (nouvelleLongueur / 2);
		//return true;
	//}
	//else
		//return false;
...
void NoeudMur::mettreAjourPosition()
{
	...
	pointDepart_.x = cos(angle) * (base_Debut.x) - sin(angle) * (base_Debut.y) + positionRelative_.x;
	pointDepart_.y = sin(angle) * (base_Debut.x) + cos(angle) * (base_Debut.y) + positionRelative_.y;

	pointFin_.x = cos(angle) * (base_Fin.x) - sin(angle) * (base_Fin.y) + positionRelative_.x;
	pointFin_.y = sin(angle) * (base_Fin.x) + cos(angle) * (base_Fin.y) + positionRelative_.y;
}
...
void NoeudMur::tourner(double angle)
{
	...
	double demiLongeurFin = std::sqrt(std::pow((pointFin_[0] - position[0]), 2) + std::pow((pointFin_[1] - position[1]), 2));
	double demiLongeurDepart = std::sqrt(std::pow((pointDepart_[0] - position[0]), 2) + std::pow((pointDepart_[1] - position[1]), 2));
